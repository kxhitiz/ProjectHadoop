...
class MongoConnector extends DatabaseConnector {
  ...
  val db = connection("smartapp_development")
  ...
  override def insert(movie : BSONDocument ): Unit = {
    ...
    future.onComplete {
      ...
      case Success(lastError) => {
        println(s"successfully inserted document ${movie.get("movieId")} with lastError = ${lastError}")
      }
    }
  }
  ...
  override def list(name : String): List[Movie] = {
    ...
    val query = BSONDocument("movieId" -> BSONInteger(name.toInt))
